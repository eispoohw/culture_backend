...
class ItemQuantity(models.Model):
    ...
    label = models.CharField(
        max_length=2048, null=True, blank=True, help_text='Название единиц измерения, например "кг" или "шт"'
    )
...
class RenderedCartItem(models.Model):
    ...
    quantity = models.OneToOneField(
        ItemQuantity, on_delete=models.CASCADE, related_name='quantity', help_text='Количество товара в заказе'
    )
    subtotal = models.CharField(
        null=True, blank=True, validators=[DecimalValidator], help_text='Суммарная цена за позицию без учета скидок'
    )
